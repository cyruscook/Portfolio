...
function writeVars(){
	...
	if(port_data.cards.length > 0){
		...
		port_data.cards.forEach(function(card){
			...
			if(get_if_exist(card.img) != undefined){
				imagecode = `<center><img class="card-img-top" src="${card.img.url}" style="margin-top: calc( (${card.img.height}px / 2 )  * -1); background-color: #666; width: ${card.img.width}; height: ${card.img.width}; border-radius: ${card.img["border-radius"]};"></img></center>`;
